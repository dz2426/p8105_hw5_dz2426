...
q2_df %>% 
  ...
  labs(x = "Week", 
       y = "Observation",
       title = "Figure 1. Observations on each subject over time"
    ) +
...
```{r}
# Map all 6 beta1 values into the output
sim_result_beta1s = 
  tibble(beta1s = c(1:6)) %>% 
  mutate(
...
**Comments:**  
The plot shows a positive association between the power of test and effect size. The power increases as the effect size increases.
...
ggplot(q3_plot2, aes(x = beta1s)) +
  ...
  scale_x_continuous(breaks = seq(1, 6, by = 1)) +
  scale_y_continuous(breaks = seq(1, 6, by = 0.2)) + 
  labs(
    x = "Beta1", y = "Average beta1 estimate", 
    title = "Figure 3. Association between average beta1 estimate and true beta1",
    caption = "Simulation iterations: 10000"
  ) +
